...
export class UserController {
  ...
  @ApiOperation({description: 'Find by user id properties'})
  async findProperties(@Param('id', ObjectIdValidationPipe) id: string): Promise<IUser> {
    return await this.userService.findProperties(id);
  }

  @Get(':id/visits')
  @ApiImplicitParam({name: 'id', description: 'User id'})
  @ApiOperation({description: 'Find by user id visits'})
  async findVisits(@Param('id', ObjectIdValidationPipe) id: string): Promise<IUser> {
    return await this.userService.findVisits(id);
  }
